...
export default {
    ...
    setup() {
        ...
        const validationSchema = yup.object().shape({
            ...
            message: yup
                .string()
                .required("Ваш отзыв обязателен для заполнения"),
            agree_to_process_personal_data: yup
                .boolean()
                .oneOf(
                    [true],
        ...
        const submitForm = async () => {
            ...
            try {
                await validationSchema
                    ...
                    .then(() => {
                        axios
                            .post("/api/reviews/store", form.value)
                            .then((res) => {
                                closeModal();
                                appStore.showToast(
                                    "success",
                                    "Отзыв успешно отправлен. Он будет опубликован после модерации."
                                );
                                resetForm();
                                loading.value = false;
                            })
                ...
                if (validationError.inner) {
                    ...
                    appStore.showToast(
                        "error",
                        "Произошла ошибка при валидации формы"
                    );
                }
            }
        };
        ...
        <form @submit.prevent="submitForm" class="space-y-4">
            ...
            <div>
                ...
                <textarea
                    ...
                    required
                ></textarea>
                <p v-if="errors.text" class="mt-1 text-sm text-red-600">
                    {{ errors.text }}
                </p>
            </div>
            ...
            <div class="flex items-start">
                ...
                <div class="ml-3 text-sm">
                    ...
                    >
                    <p v-if="errors.privacy" class="mt-1 text-sm text-red-600">
                        {{ errors.privacy }}
                    </p>
                </div>
            </div>
